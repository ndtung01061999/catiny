...
const Messages = (props: IMessageProps) =>
{
  ...
  const sendMessage = (data, details) =>
    connection?.then(() =>
      stompClient?.send(
  ...
  const subscribe = () =>
    connection.then(
      () => (subscriber = stompClient.subscribe(`/topic/video-call/groupx/${props.account.login}/${uuid}`, data => process(data)))
    );
    ...
    const thisUserIsSender = () => (
      <>
        ...
        <Media
          className="mr-2"
          object
          src="https://i.pinimg.com/564x/e1/24/04/e124041a63e3a93437603fbb93255169.jpg"
          alt="Generic placeholder image"/>
      </>
    );
